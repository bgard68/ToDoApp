...
public class BreachedPasswordTests
{
    ...
    private static TodoApp.Infrastructure.Authentication.PasswordHasher Hasher => new();
    ...
    public async Task A_breached_password_does_not_create_an_account()
    {
        ...
        db.NewContext().Users.Should().BeEmpty("the check runs before anything is persisted");
    }
    ...
    public async Task The_checker_failing_open_does_not_block_registration()
    {
        ...
        using var db = new TestDatabase();
        var failOpen = new FakeBreachedPasswordChecker { Breached = false };
